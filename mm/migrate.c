				...
				enum migrate_mode mode)
{
	 ...
	 */
	if (rc == MIGRATEPAGE_SUCCESS) {
		bool is_thp = PageTransHuge(page) && !PageHuge(page);
		int nr_subpages = thp_nr_pages(page);
		count_vm_events(VM_MIGRATE_BASE + page_to_nid(page) * KMIG_MAX_NUMA_NODES + page_to_nid(newpage), is_thp ? nr_subpages : 1);
				...
				struct page *page, int node)
{
	...
	count_vm_events(VM_MIGRATE_BASE + page_to_nid(page) * KMIG_MAX_NUMA_NODES + page_to_nid(new_page), HPAGE_PMD_NR);
